import { Project, SourceFile, ClassDeclaration, MethodDeclaration, ConstructorDeclaration } from 'ts-morph';
...
export class ServiceExtractor {
  ...
  private extractMethods(classDeclaration: ClassDeclaration): MethodInfo[] {
    ...
    for (const method of methodDeclarations) {
      ...
      if (method.getKind() === 173 || method.hasModifier('private')) { // Constructor
        continue;
      }
      ...
      methods.push({
        ...
        isPublic: method.hasModifier('public') || !method.hasModifier('private') && !method.hasModifier('protected')
      });
    }
  ...
  private extractParameters(method: MethodDeclaration): ParameterInfo[] {
    ...
    for (const param of methodParameters) {
      ...
      parameters.push({
        ...
        decorator: decoratorNames.length > 0 ? decoratorNames[0] : undefined,
