...
export class Logger {
   ...
   */
  logResults(controllers: ControllerInfo[], services: ServiceInfo[], analysisTime: number): void {
    this.logHeader();
   ...
   */
  private logSummary(controllers: ControllerInfo[], services: ServiceInfo[], analysisTime: number): void {
    const totalControllerMethods = controllers.reduce((sum, c) => sum + c.methods.length, 0);
    const totalServiceMethods = services.reduce((sum, s) => sum + s.methods.length, 0);
    
    if (this.options.colorOutput) {
      console.log(chalk.green(`📁 Controllers Found: ${controllers.length}`));
  ...
  private logService(service: ServiceInfo): void {
    ...
    if (service.dependencies.length > 0) {
      const dependencies = this.options.colorOutput
        ? chalk.cyan(`   Dependencies: [${service.dependencies.join(', ')}]`)
   ...
   * Log a single method
   */
  private logMethod(method: any, isLast: boolean, type: 'controller' | 'service'): void {
    const prefix = isLast ? '└── ' : '├── ';
    ...
    if (type === 'controller' && method.decorators.length > 0) {
      ...
      const decorators = method.decorators.map((d: string) => `@${d}()`).join(', ');
   ...
   * Log final summary
   */
  logFinalSummary(controllers: ControllerInfo[], services: ServiceInfo[], analysisTime: number): void {
    const totalControllerMethods = controllers.reduce((sum, c) => sum + c.methods.length, 0);
    const totalServiceMethods = services.reduce((sum, s) => sum + s.methods.length, 0);
    ...
    if (this.options.colorOutput) {
      ...
      console.log(chalk.white(`   • Total Controllers: ${controllers.length}`));
      console.log(chalk.white(`   • Total Services: ${services.length}`));
      console.log(chalk.white(`   • Total Controller Methods: ${totalControllerMethods}`));
      console.log(chalk.white(`   • Total Service Methods: ${totalServiceMethods}`));
      console.log(chalk.green(`   • Analysis completed in ${analysisTime.toFixed(2)}s`));
    } else {
      console.log('📊 Summary:');
      console.log(`   • Total Controllers: ${controllers.length}`);
      console.log(`   • Total Services: ${services.length}`);
      console.log(`   • Total Controller Methods: ${totalControllerMethods}`);
